...
public class Main {
    ...
    public static Map<Integer, Queue<int[]>> doorList;
    public static int keyBit;
        ...
        while(!q.isEmpty()) {
            ...
            for(int d = 0; d < 4; d++) {
                ...
                if(map[nx][ny] - 'A' >= 0 && map[nx][ny] - 'A' <= 25) {
                    int doorBit = 1 << (map[nx][ny] - 'A');
                    if((keyBit & doorBit) != doorBit) {
                        putDoorQueue(nx, ny, doorBit);
                        continue;
                    }
